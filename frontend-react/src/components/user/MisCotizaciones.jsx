...
export default function MisCotizaciones() {
  ...
  // use cached fetch so repeated renders don't hit the API
  const apiUrl = token ? 'http://localhost:3000/api/cotizaciones' : null;
  const { data: fetchedData, loading: fetching, error: fetchError } = useCachedFetch(
    apiUrl,
  ...
  useEffect(() => {
    if (fetchedData) setCotizaciones(fetchedData || []);
    if (fetchError) {
